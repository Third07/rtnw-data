const CONFIG = {
    // Absolute bases so the simulator works from GitHub Pages (or any host)
    dataBasePath: "https://third07.github.io/rtnw-data/data/",
    iconBasePath: "https://third07.github.io/rtnw-data/data/images/"
}, withAssetVersion = window.withAssetVersion || (e => e),
// Only locales that actually exist under /data/ right now
SUPPORTED_LOCALES = [ "en-US" ],
FALLBACK_LOCALE = "en-US";

function detectLocale() {
    const e = new URLSearchParams(window.location.search).get("lang"),
        t = localStorage.getItem("ro_lang"),
        n = document.documentElement.getAttribute("lang"),
        l = Array.isArray(navigator.languages) ? navigator.languages : [],
        i = [ e, (navigator.language || "").trim(), ...l, t, n ],
        o = e => SUPPORTED_LOCALES.some(t => t.toLowerCase() === String(e).toLowerCase());
    for (const e of i) {
        if (!e) continue;
    ...
    for (const e of i) {
        ...
        if ("zh" === t) {
            const zh = SUPPORTED_LOCALES.find(e => e.toLowerCase().startsWith("zh-"));
            if (zh) return zh;
        }
        if (("id" === t || "in" === t) && o("id-ID")) return "id-ID";
    }
    return o(FALLBACK_LOCALE) ? FALLBACK_LOCALE : SUPPORTED_LOCALES[0] || "en-US";
}

let ACTIVE_LOCALE = detectLocale();

localStorage.setItem("ro_lang", ACTIVE_LOCALE), document.documentElement.setAttribute("lang", ACTIVE_LOCALE);

function dataUrl(path) {
    return CONFIG.dataBasePath + String(path || "").replace(/^\//, "");
}

const GRID_COLUMNS = 5,
    ICON_PATHS_URL = dataUrl("icon_paths.json"),
    SKILLS_INDEX_URL = () => dataUrl(`skills_index_${ACTIVE_LOCALE}.json`),
    JOB_DATA_URL = e => dataUrl(`jobs_${ACTIVE_LOCALE}/${e}.json`),
    JOB_SELECTION_PLACEHOLDER_BY_LOCALE = {
    "zh-TW": "選擇職業...",
...
async function loadSkillsIndex() {
    ...
    const tryLocales = [ ACTIVE_LOCALE ];
    if (ACTIVE_LOCALE !== FALLBACK_LOCALE) tryLocales.push(FALLBACK_LOCALE);
    let lastError = null;
    for (const locale of tryLocales) {
        try {
            const url = dataUrl(`skills_index_${locale}.json`);
            const e = await fetch(withAssetVersion(url));
            if (!e.ok) {
                lastError = new Error(`Failed to load skills index (${locale}): ${e.status}`);
                continue;
            }
            const t = await e.json(), n = t.jobs || t;
            ACTIVE_LOCALE = t.locale || locale;
            state.locale = ACTIVE_LOCALE;
    ...
    throw lastError || new Error("Failed to load skills index.");
}
